...
#define  ATL_DECL_ 1
#include Mstr(Mjoin(ATLAS_PRE,ipnek_view.h))
#undef ATL_DECL_
...
void Mjoin(PATL,ipnekInfoPop)  /* populate ip wt given params */
(
   ipinfo_t *ip,        /* output */
...
)
{
   ...
   ip->idx = idx;
   ATL_GetVWipnekInfo(idx, spmf, k, k, kb, imm, icpA, icpB, icpC, k,
                      kbmin, k);
   ip->idxK = imm;
   icpA += icpA;
   icpB += icpB;
 ...
 */
   if (NB+NB > N && MB+MB > M)
   {
      ...
      if (ATL_MulBySize(mb*nb + 2*(mb+nb)*KB) <= LLPC_SZ)
      {
         MB = mb;
         NB = nb;
         KB = NB; /* KB must be equal to NB for nek */
      }
   }
   if (MB >= M)
   {
   ...
   Mjoin(PATL,ipnekInfoPop)(ip, idx, TA, TB, M, N, K, lda, ldb, ldc, alpha,
