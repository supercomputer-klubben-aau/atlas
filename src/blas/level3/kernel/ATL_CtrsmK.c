...
#ifdef Left_
static void trsmLU_2(ATL_CSZT  N, const TYPE *A, TYPE *B, ATL_CSZT ldb)
/*
 ...
 */
{
   ...
   TYPE t0, p0;
   ATL_CSZT ldb2=ldb+ldb;
   TYPE *bn=B+ldb2;
   ATL_CSZT  pfd=ldb2+ldb2;
   int j;
...
static void trsmLU_3(ATL_CSZT  N, const TYPE *A, TYPE *B, ATL_CSZT ldb)
/*
 * 'Left, 'Upper', with 1 col prefetch, written with all dependencies shown,
 * so that compiler can optimize.
...
static void trsmLU_4(ATL_CSZT  N, const TYPE *A, TYPE *B, ATL_CSZT ldb)
/*
 * 'Left, 'Upper', with 1 col prefetch, written with all dependencies shown,
 * so that compiler can optimize.
...
static void trsmLL_2(ATL_CSZT  N, const TYPE *A, TYPE *B, ATL_CSZT ldb)
/*
...
static void trsmLL_3(ATL_CSZT  N, const TYPE *A, TYPE *B, ATL_CSZT ldb)
/*
 * 'Left', 'Lower', with 1 column prefetch, written with all dependencies
 * shown, so that the compiler can optimize.
 * A is known to be 3x3, with 1/alpha already applied, diagonals already
...
static void trsmLL_4(ATL_CSZT  N, const TYPE *A, TYPE *B, ATL_CSZT ldb)
/*
 * 'Left', 'Lower', with 1 column prefetch, written with all dependencies
 * shown, so that the compiler can optimize.
 * A is known to be 4x4, with 1/alpha already applied, diagonals already
...
#ifdef Right_
static void trsmRU_2(ATL_CSZT  M, const TYPE *A, TYPE *B, ATL_CSZT ldb)
/*
...
static void trsmRU_3(ATL_CSZT  M, const TYPE *A, TYPE *B, ATL_CSZT ldb)
/*
 * 'Right', 'Upper', written with all dependencies shown, so that the
...
static void trsmRU_4(ATL_CSZT  M, const TYPE *A, TYPE *B, ATL_CSZT ldb)
/*
 * 'Right', 'Upper', written with all dependencies shown, so that the
...
static void trsmRL_2(ATL_CSZT  M, const TYPE *A, TYPE *B, ATL_CSZT ldb)
/*
...
static void trsmRL_3(ATL_CSZT  M, const TYPE *A, TYPE *B, ATL_CSZT ldb)
/*
 * 'Right', 'Lower', written with all dependencies shown, so that the
 * compiler can optimize.  A is known to be 3x3, with 1/alpha already applied,
 * diagonals already inverted.
...
static void trsmRL_4(ATL_CSZT  M, const TYPE *A, TYPE *B, ATL_CSZT ldb)
/*
 * 'Right', 'Lower', written with all dependencies shown, so that the
 * compiler can optimize.  A is known to be 4x4, with 1/alpha already applied,
 * diagonals already inverted.
...
static void trsmcpUN
   (ATL_CSZT  N, const TYPE *alpha, const TYPE *A, ATL_CSZT lda, TYPE *C)
/*
 * copies Upper, NoTranspose matrix to Upper, NoTrans matrix, applying 1/alpha
 */
{
   ATL_CSZT lda2 = lda+lda, N2 = N+N;
   const TYPE *a=A+lda2;
   TYPE *c=C+N2;
   TYPE tmp[2];
   int j;
...
static void trsmcpUT
   (const enum ATLAS_TRANS Trans, ATL_CSZT  N, const TYPE *alpha,
    const TYPE *A, ATL_CSZT lda, TYPE *C)
/*
 ...
 */
{
   ...
   TYPE tmp[2];
   int j;
   void (*move)(ATL_CSZT  N, const SCALAR alpha, const TYPE *X, ATL_CSZT  incX,
                TYPE *Y, ATL_CSZT  incY);
...
static void trsmcpLT(const enum ATLAS_TRANS Trans, ATL_CSZT  N,
                     const TYPE *alpha, const TYPE *A, ATL_CSZT lda, TYPE *C)
/*
 * copies Lower, Trans matrix to Upper, Notrans matrix, applies 1/alpha
 */
{
   ATL_CSZT lda2 = lda+lda+2, N2 = N+N+2;
   const TYPE *a=A+2;
   TYPE *c=C+N+N;
   TYPE tmp[2];
   int j;
...
static void trsmcpLN(ATL_CSZT  N, const TYPE *alpha, const TYPE *A,
                     ATL_CSZT lda, TYPE *C)
/*
...
void Mjoin(PATL,CtrsmKR)
   (enum ATLAS_UPLO Uplo, enum ATLAS_TRANS Trans, enum ATLAS_DIAG Diag,
    ATL_CSZT  N, ATL_CSZT  M, const SCALAR alpha, const TYPE *A, ATL_CSZT lda,
    TYPE *B, ATL_CSZT ldb)
#else
void Mjoin(PATL,CtrsmKL)
   (enum ATLAS_UPLO Uplo, enum ATLAS_TRANS Trans, enum ATLAS_DIAG Diag,
    ATL_CSZT  M, ATL_CSZT  N, const SCALAR alpha, const TYPE *A, ATL_CSZT lda,
    TYPE *B, ATL_CSZT ldb)
#endif
{
   TYPE tmp[2], ra, ia;
