 ...
 */
{
   char *frm = "./xspew -v %d -O %d -A %d -m %d -s %d -V %d -b %d -f %d -d s '%s' -d b '%s' -D c '%s' -D f '%s' %s -Si archdef %d -Si ieee %d -Si bozol1 %d -Si latune %d -Si nof77 %d -Si fko %d -Si indthr %d -o Make.inc";
   ...
   i = sprintf(ln, frm, verb, OS, arch, mhz, asmb, vecexts, ptrbits, l2size,
               ...
               SI_IS_TRUE(SIflag,SI_FKO), SI_IS_TRUE(SIflag,SI_INDTHR));
   free(compsflags);
   if (tids)
   {
      int k;
