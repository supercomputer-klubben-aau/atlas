 ...
 */
{
   ...
   else if ((vecexts & (1<<ISA_AVXZ)))
      vp = "-mavx512f -mavx512dq -mfma";
 ...
 */
{
   int i, iexa=0, imaj=0, inew=0, iwrk=0, mingap=0, newmaj=0, newmin=0;
   int wrkmaj=0, wrkmin=0;
   ...
   for (i=0; gccs[i]; i++)
   {
      ...
      if (!USEMINGW && CompIsMinGW(gccs[i]))
         continue;
      if (CompTest(verb, targ, GCC_, gccs[i], "-O"))  /* if compiler doesn't */
         continue;                                    /* work, skip it! */
      ...
      {
         if (NameIsExactGcc(gccs[i]))  /* found exact version with */
            return(i+1);               /* "gcc" as name, return it! */
         if (!iexa)  /* always take 1st non-"gcc" exact match */
            iexa = i+1;
      }
      if (iexa)     /* once we find exact match */
         continue;  /* no need to scope non-preferred versions */
/*
 *    If its a newer version of the major release we want, we'll keep the
 *    the version closest in minor to the asked, and use exact gcc name
 *    as tiebreak;
 */
      if (major == GMAJOR && minor > GMINOR)
      {
         if (imaj) /* choose amongst two candidates by minimizing gap */
         {
            int gap = GMINOR - minor;
            if (gap < mingap)
            {
               imaj = i+1;
               mingap = gap;
            }
            else if (gap == mingap && NameIsExactGcc(gccs[i]))
               imaj = i+1;
         }
      ...
      if (imaj)    /* if we have newer major release */
         continue; /* don't look for lesser matches */
/*
 *    If we can't find same major release with newer minor, just take newest
 *    compiler we can find, and break ties by "gcc", then path order
 */
      if (major > GMAJOR)
      {
         if (!inew)
         ...
         else if (major == newmaj && minor == newmin && NameIsExactGcc(gccs[i]))
            inew = i + 1;
         else if (major > newmaj || (major == newmaj && minor > newmin))
         {
            inew = i + 1;
            newmaj = major;
            newmin = minor;
      ...
      if (inew)     /* if we've found a newer compiler than requested */
         continue;  /* don't look at other compilers */
      if (!iwrk)
      {
         iwrk = i+1;
         wrkmaj = major;
      ...
      else if (major == wrkmaj && minor == wrkmin && NameIsExactGcc(gccs[i]))
         iwrk = i + 1;
      else if (major > wrkmaj || (major == wrkmaj && minor > wrkmin))
      {
         iwrk = i+1;
         wrkmaj = major;
         wrkmin = minor;
